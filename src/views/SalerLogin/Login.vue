...
import { ref } from 'vue'
import { verifyEntryToken } from '@/api/salerApi/auth'
...
const isCanShowRegister = ref(false)
...
if (!route.query.token) {
  router.push({ name: 'NoFind' })
}
const token = ref(route.query.token as string)
verifyEntryToken(route.query.token as string)
  .then((res) => {
    if (res.data.type === 'register') {
      isCanShowRegister.value = true
    }
  })
  .catch(() => {
    router.push({ name: 'NoFind' })
      ...
      <div class="flex-1 p-30px <sm:p-10px dark:bg-v-dark relative">
        ...
        <Transition appear enter-active-class="animate__animated animate__bounceInRight">
          ...
          >
            <LoginForm
              :token="token"
              :is-can-show-register="isCanShowRegister"
            ...
            <RegisterForm
              :token="token"
              v-if="!isLogin && !isRegisterSuccess && isCanShowRegister"
              class="p-20px h-auto m-auto <xl:(rounded-3xl light:bg-white)"
