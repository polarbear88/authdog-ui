...
import { setDisableForceUpgrade, setEnableForceUpgrade, setDownloadUrl } from '@/api/application'
  ...
  <div
    ><Descriptions :collapse="false" title="版本信息" :data="app" :schema="schema">
      <template #forceUpgrade="data">
        <ElSwitch @change="onSetEnableForceUpgrade" v-model="data.row.forceUpgrade" />
      </template>
      <template #downloadUrl="data">
        {{ data.row.downloadUrl ? data.row.downloadUrl : '未设置' }}
        <ElButton @click="onSetDownloadUrl" type="primary" link
