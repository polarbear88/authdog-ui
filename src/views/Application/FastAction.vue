...
const onDelete = () => {
  ...
  ElMessageBox.confirm(`确定要删除？`, '删除充值卡', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    deleteRechargeCardByCards(currentId.value, cardsArr).then((res) => {
  ...
  <div>
    <ContentWrap title="充值卡快速操作">
      ...
      <div style="margin-top: 15px">
        <span>应用</span>
        <ApplicationSelect style="margin-top: 10px" v-model="currentId" :zeroname="''" />
        <br />
        <span>充值卡号</span>
        <ElInput
